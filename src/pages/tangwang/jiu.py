...
from PyQt5.QtGui import QPainter, QPixmap, QPen, QColor, QBrush, QPainterPath, QPixmap, QPainter, QDesktopServices
from PyQt5.QtWidgets import QFrame, QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QFileDialog, QTextEdit
from PyQt5.QtCore import Qt, QRectF, QPointF, QLineF, QUrl
...
class ImageViewer(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if self.enable_drawing:
                self.rect_item = self.scene.addRect(QRectF(self.start_point, self.start_point), QPen(QColor(255, 0, 0), 3))
            elif self.measuring_distance:
                self.line_item = self.scene.addLine(QLineF(self.start_point, self.start_point), QPen(QColor(255, 0, 0), 3))
    ...
    #             # 在此处添加代码以输出鼠标点击和抬起之间的距离
    #             mouse_distance = QLineF(self.start_point, end_point).length()
    #             print("Distance between mouse click and release:", mouse_distance)
    #         elif self.drawing_path and self.path_item:
    #             # 不自动闭合路径
    #             path_length = self.path.length()
    #             print("Length of the drawn path:", path_length)
    #         elif self.filling_path and self.path_item:
    #             # 计算被框选部分的面积
    #                 end_point = self.mapToScene(event.pos())
    #                 self.path.lineTo(end_point)
...
"")
        ...
        sizePolicy.setHeightForWidth(self.toolButton_5.sizePolicy().hasHeightForWidth())
        self.toolButton_5.setSizePolicy(sizePolicy)
        self.toolButton_5.setStyleSheet("font: 12pt \"黑体\";\n"
...
"")
        ...
        # self.BodyLabel_8 = BodyLabel(self.verticalFrame)
        # sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred)
        # sizePolicy.setHorizontalStretch(0)
        # sizePolicy.setVerticalStretch(2)
        ...
        #             r'C:\Users\cyq\Desktop\new\报告单文件夹'))
        self.PrimaryPushButton.clicked.connect(
            partial(self.replace_images, r"数码宝贝.docx",
                    r'报告单文件夹'))
    ...
    def open_word_file(self):#用它打开特定文件（模板）的函数
        ...
        word_file_path = r'数码宝贝.docx'  # 请替换为实际的文件路径
        # 将文件路径转换为 QUrl
    ...
    def replace_images(self,doc_path,file_path):
        ...
        for rel in doc.part.rels.values():
            if "image" in rel.reltype:
                ...
                # elif os.path.exists(new_image_path2) and image_path == 'media/image1.png':
                #     rel.target_part._blob = open(new_image_path2, 'rb').read()
                #     print(f"Image replaced with: {new_image_path}")
                else:
            ...
            for run1 in paragraph.runs:
                ...
                if run1.text:
                    run1.text = run1.text.replace('a',
                                                  '在PyQt中使用QDesktopServices.openUrl方法可能无法在某些平台上触发默认关联程序。这可能是因为在某些平台上，直接打开Word文档可能会导致安全性问题。')
                    # 替换 'time' 为当前日期
                    if 'time' in run1.text:
                        run1.text = run1.text.replace('time', current_date)

        # new_doc_path = os.path.splitext(doc_path)[0] + "_modified.docx"
        # doc.save(new_doc_path)#文件保存在当前目录
        # output_folder = r'C:\Users\cyq\Desktop\new\目标文件夹'  # 替换为你的目标文件夹路径
        ...
        # QDesktopServices.openUrl(QUrl.fromLocalFile(new_doc_path))

        # 使用 subprocess 打开默认关联程序
        try:
        ...
        # 点击这个函数之后能够将照片显示在指定位置
        # 点击完生成报告之后会删除当前（第一个最前面的照片）
        # self.delete_first_photo()
        self.pre_showImage()
    ...
    def on_label1_clicked(self, event):
        ...
        print(fields)
        self.BodyLabel.setText("姓名: {} 年龄：{} 病变区域：{}".format(fields[1], fields[2], fields[3]))

        # print(fields[1])
        # self.BodyLabel.setText(ex_image_name)
        # 样式表设置
        style_sheet = "QLabel { color: blue; font-size: 27px; font-family: Arial; font-weight: bold; text-align: center; }"
        self.BodyLabel.setStyleSheet(style_sheet)


    def on_label2_clicked(self, event):
        # 当 label1 被点击时，将图片从 label1 转移到 label2
        # self.showImage2(self.BodyLabel_6, os.path.join(dir, image_files[1]))  # 在两个主要label上显示同一张图片
        # print(image_files[1])

        # self.showImage2(self.BodyLabel_3, os.path.join(dir, image_files[1]))
        #
        # print(os.path.join(dir, image_files[1]))
        # pixmap1 = self.BodyLabel_6.pixmap()
        # self.BodyLabel_3.setPixmap(pixmap1)
        # # 设置字体大小为 14
        # self.text_edit.setFontPointSize(14)

        # self.BodyLabel.setText(
        #     "这种情况由于已经导致视网膜表面循环的障碍，所以，很容易滋生新生血管。对于这种情况要及早到医院进行眼底检查，必要的时候也可以考虑行视网膜激光光凝术，能够有效的抑制新生血管形成，并且促进已经产生的新生血管消退。")
        # 加载图片
    ...
    def yuce(self):
        ...
        self.text_edit.setText(f"Class: {class_values}, Probability: {probability_values}, Prediction: {preclass_values}")
    ...
    def retranslateUi(self, Form):
        ...
        self.ToolButton_2.setText(_translate("Form", "历史报告"))
        ...
        self.ToolButton_8.setText(_translate("Form", "设置（模式更换）"))
