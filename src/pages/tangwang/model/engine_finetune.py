...
@torch.no_grad()
def predict_single_image(image_path, model, device, num_class, input_size):
    t = []
    ...
    with torch.no_grad():
        ...
        _, prediction_decode = torch.max(prediction_softmax, 1)
    # 获取每个类别的概率
    preclass = prediction_decode.cpu().detach().numpy()
    probabilities = prediction_softmax[0].cpu().numpy()
    return probabilities, preclass
